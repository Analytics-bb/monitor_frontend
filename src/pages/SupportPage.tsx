...
export function SupportPage() {
  ...
  const limitReachedKey = useMemo(() => {
    if (!isHistoryLimitReached) {
      return null
    }
    return `${historyMessageLimit}:${historyMessageCount}`
  }, [historyMessageCount, historyMessageLimit, isHistoryLimitReached])

  const [declinedLimitKey, setDeclinedLimitKey] = useState<string | null>(null)
  const historyLimitDeclined =
    limitReachedKey !== null && declinedLimitKey === limitReachedKey
  ...
  const handleDeclineHistoryReset = () => {
    if (limitReachedKey) {
      setDeclinedLimitKey(limitReachedKey)
    }
    setPendingAttachments([])
  }
