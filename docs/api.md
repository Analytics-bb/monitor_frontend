...
**Assistant message (deep):** structured text с emoji-секциями (`📈 Детекция`, `🔧 Действия`, `🎯 Итог`, …). Перед `pending_action` последний `assistant` всегда содержательный (не голый approve/reject).
...
Human-in-the-loop: MCP execute **только** после `approve`. `POST .../messages` и первый turn на `open` — только LLM propose (без MCP).

**Типичный флоу (SPA `/deep/{audit_id}`):**

1. `deep_chat_state=not_started` → `POST .../chat/open` (синхронно: session seed + первый LLM turn).
2. Snapshot: `messages[]` = `[system, assistant]`; hypothesis UI — из `system.conclusion`, не из `user`.
3. Если агент предложил tool → `state=awaiting_approval`, `pending_action` заполнен; composer заблокирован.
4. `POST .../approve` → MCP execute + assistant follow-up; `POST .../reject` → без execute.
5. Дальнейшие реплики оператора → `POST .../messages` (propose); polling `GET .../chat` (1–2 с в `active`, 3–5 с в `awaiting_approval`).

Один `pending_action` за раз; в тексте агента до 3 пунктов в `🔧 Действия`, в API — один tool на шаг.
...
1. Создаёт `session_id`, пишет `system` message (audit seed, см. выше).
2. Запускает первый LLM propose (скрытый user-триггер, не попадает в `messages[]`).
3. Добавляет `assistant` — structured summary.
4. `state`: `active` (без tool) или `awaiting_approval` (если агент предложил tool).

Повторный вызов при `active` / `awaiting_approval` → `200` с текущим snapshot без мутаций.
...
**Ошибки:** 404 `audit_entry_not_found`, 409 `deep_chat_terminal_state`, 409 `budget_exceeded`, 409 `tool_not_whitelisted`, 503 `deep_analyst_not_initialized`

**Примечание:** open может быть долгим (ожидание первого LLM-ответа). FE показывает thinking до получения `200`.

---

### POST /api/deep/cases/{audit_id}/chat/messages

Отправить сообщение оператора — LLM propose **без** execute MCP.
...
Подтвердить pending MCP step: budget check + **MCP execute** + usage + assistant follow-up.
...
User-centric support-чат с auto-execute MCP (без approve как в deep). Владелец — `src/support_agent/`. `user_id` из сессии (`get_current_user`); клиент не передаёт `user_id`. Один чат на пользователя (lazy create на `GET`).
...
| `POST`   | `/api/deep/cases/{audit_id}/chat/open`                   | M16    | Bearer*      | Open + первый LLM turn       |
| `POST`   | `/api/deep/cases/{audit_id}/chat/messages`               | M16    | Bearer*      | Propose (без MCP execute)    |
| `POST`   | `/api/deep/cases/{audit_id}/actions/{action_id}/approve` | M16    | Bearer*      | Approve + MCP execute        |
