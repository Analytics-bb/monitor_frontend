...
- **Аутентификация:** **M19** — opaque session Bearer на всех `/api/*`, кроме allowlist (см. § Auth); модуль `docs/modules/module-19-session-auth.md`
...
Источники: `src/*/routes.py`, Pydantic-модели в `src/*/models.py`, модульные документы в `docs/modules/`, планы M19/M18 — `.cursor/plans/R2/module-19-session-auth.plan.md`, `.cursor/plans/R2/module-18-support-agent.plan.md`.

> **M18:** секция Support chat описывает **целевой контракт** (план `draft`, wire в `main.py` — в разработке). **M19 (Auth)** реализован: все эндпоинты ниже, кроме allowlist, требуют `Authorization: Bearer`.
...
| **Путь**        | `/api/status`                  |
| **Вход**        | —                          |
| **Auth (M19)**  | **не требуется** (allowlist) |
...
MVP session auth для всего `anomaly-api`. Владелец — **`src/auth/`** (M19). Opaque Bearer token в Mongo `auth_sessions`; seed user из env (`AUTH_ADMIN_USERNAME` / `AUTH_ADMIN_PASSWORD`, dev default `admin`/`admin`). Документ модуля: `docs/modules/module-19-session-auth.md`.

### Global guard

Все `/api/*` требуют заголовок `Authorization: Bearer <token>`, **кроме allowlist:**
...
| Метод    | Путь                                                 | Модуль | Auth (M19)   | Назначение                   |
| -------- | ---------------------------------------------------- | ------ | ------------ | ---------------------------- |
| `GET`    | `/api/status`                                        | M10    | public       | Агрегированный observability |
| `POST`   | `/api/status/tick`                                   | M5     | Bearer       | Ручной тик (debug)           |
| `POST`   | `/api/auth/login`                                    | M19    | public       | Login → token                |
| `POST`   | `/api/auth/logout`                                   | M19    | Bearer       | Invalidate session           |
| `GET`    | `/api/auth/me`                                       | M19    | Bearer       | Текущий пользователь         |
...
| `POST`   | `/api/deep/cases/{audit_id}/chat/open`                   | M16    | Bearer*      | Открыть deep chat            |
| `POST`   | `/api/deep/cases/{audit_id}/chat/messages`               | M16    | Bearer*      | Сообщение в chat             |
| `POST`   | `/api/deep/cases/{audit_id}/actions/{action_id}/approve` | M16    | Bearer*      | Approve MCP step             |
...
\* Колонка **Auth (M19):** `public` — без Bearer; `Bearer` — обязателен. Реализовано в `main.py` (global guard).
...
- Module doc M19: `docs/modules/module-19-session-auth.md`
- План M19: `.cursor/plans/R2/module-19-session-auth.plan.md`
- План M18 (support chat): `.cursor/plans/R2/module-18-support-agent.plan.md`
