...
- [x] Light + dark палитры (§Палитра) и шрифты в `index.css`; contrast ≥4.5:1 в обеих темах.
- [x] Theme toggle: default light, persist `localStorage`, без flash on load.
- [x] `StatusBadge` покрывает monitoring + deep chat варианты; страницы не используют локальные status-цвета.
- [x] `AppLayout` + `Sidebar` + sonner Toaster смонтированы; маршруты совпадают с M17.
- [x] `api/client` — base URL, timeout, GET-retry, Bearer M19; Zod error envelope.
- [x] `usePolling` — unmount, interval switch, visibility; unit-тесты зелёные.
- [x] Fixtures для 5 типов сэмплов доступны в `src/api/fixtures/`.
- [x] `npm run lint && npm run typecheck && npm test` проходят для shared-кода.
- [x] План апрувен пользователем.
- [x] `docs/modules/module-0-index.md`.
