...
## Что сделать
Страница `/deep/{audit_id}`: header с breadcrumb `Deep` → short id, слот StatusBadge, слот usage link, слот CaseMetaStrip, full-height ChatWindow placeholder; breadcrumb «Deep» → `/deep?${savedSearch}`.
...
`DeepChatPage`: `auditId` из `useParams`; `savedSearch` из `location.state` или `sessionStorage` (согласовать с m2 return flow).

Breadcrumb link сохраняет filter query params списка (§Провал в audit).
...
- `m2-filters` — контракт URL query для back link.
